...
def cutImage (image):
    ...
    # NOTE: when arm comes into frame, it prevents contour detection and breaks the program
    ...
    # find contours
    contours, hierarchy = cv2.findContours(hsv, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE) # find contours

    # sort contours, biggest detected is hole frame, second biggest should be the board
    contour_sizes = []
    ...
    cv2.rectangle(image, (zgornjiLevo[0], zgornjiLevo[1]),(spodnjiDesno[0],spodnjiDesno[1]),(150, 60, 0),20) # draw
    cutImage = image[zgornjiLevo[1] : spodnjiDesno[1], zgornjiLevo[0] : spodnjiDesno[0]] # cut
